...
pub async fn ensure_agent_vault(state: &Arc<AppState>, auth: &AuthInfo) -> Result<(), AppError> {
    if state
        .db
    ...
    let resp = req
        .send()
        .await
        .map_err(|e| AppError::Internal(format!("ensure-vault sidecar request failed: {}", e)))?;
    ...
    let result: VaultResponse = resp
        .json()
        .await
        .map_err(|e| AppError::Internal(format!("Failed to parse ensure-vault response: {}", e)))?;
