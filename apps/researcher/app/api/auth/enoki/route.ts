...
const MYSO_ADDRESS_REGEX = /^0x[0-9a-f]{64}$/i;
 ...
 * - Phase 1 (check): { mysoAddress } → returns existing user or needsSetup
 * - Phase 2 (register): { mysoAddress, privateKey, publicKey, accountId } → creates/updates user + session
 */
export async function POST(request: Request) {
  try {
    const body = await request.json();
    const { mysoAddress, privateKey, accountId } = body;

    if (!mysoAddress || !MYSO_ADDRESS_REGEX.test(mysoAddress)) {
      return Response.json(
        { error: "Invalid MySo address." },
        { status: 400 },
      );
    }

    // Phase 1: Check — only mysoAddress provided
    if (!privateKey && !accountId) {
      const existing = await getUserByMySoAddress(mysoAddress);
    ...
    if (!accountId || typeof accountId !== "string" || !MYSO_ADDRESS_REGEX.test(accountId)) {
      return Response.json(
    ...
    // Check if user already exists by mysoAddress (e.g. partial setup from before)
    const existing = await getUserByMySoAddress(mysoAddress);
