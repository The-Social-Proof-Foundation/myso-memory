 ...
 * Returns existing user if mysoAddress matches, otherwise creates new user
 */
...
export async function upsertWalletUser(
  ...
  const [existingUser] = await db
    .select()
    .from(users)
    .where(eq(users.mysoAddress, input.address))
    .limit(1);
