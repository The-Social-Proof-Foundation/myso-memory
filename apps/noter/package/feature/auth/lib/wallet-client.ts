/**
 * WALLET CLIENT
 * Handles MySo wallet connections using Wallet Standard
 */
...
/**
 * Detect available MySo wallets using Wallet Standard
 */
...
export async function signMessage(
  ...
  try {
    ...
    // Sign message using myso:signPersonalMessage feature
    const signFeature = wallet.features['myso:signPersonalMessage'] as {
      signPersonalMessage: (params: { message: Uint8Array; account: any }) => Promise<{ signature: string | Uint8Array }>
    } | undefined;

    if (!signFeature) {
      throw new Error("Wallet does not support myso:signPersonalMessage feature");
    }
