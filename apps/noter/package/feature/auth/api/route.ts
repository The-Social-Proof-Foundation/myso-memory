...
export const authRouter = router({
  ...
  connectEnoki: procedure
    .input(z.object({
      mysoAddress: z.string().min(1),
      privateKey: z.string().optional(),
      accountId: z.string().optional(),
    }))
    .mutation(async ({ ctx, input }) => {
      const { mysoAddress, privateKey, accountId } = input;

      try {
        // Phase 1: returning user check
        if (!privateKey && !accountId) {
          const existing = await authService.getEnokiUserByMySoAddress(ctx.db, mysoAddress);
          if (existing) {
            const sessionId = uuidv7();
            const session = await authService.createEnokiSession(ctx.db, {
              sessionId, userId: existing.id, mysoAddress,
            });
            return {
              needsSetup: false,
              user: existing,
        ...
        const user = await authService.upsertEnokiUser(ctx.db, {
          mysoAddress, delegatePrivateKey: privateKey, delegateAccountId: accountId,
        });

        const sessionId = uuidv7();
        const session = await authService.createEnokiSession(ctx.db, {
          sessionId, userId: user.id, mysoAddress,
        });
  ...
  connectDelegateKey: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        // Derive MySo address from private key
        ...
        const mysoAddress = "0x" + Array.from(new Uint8Array(addressBytes))
