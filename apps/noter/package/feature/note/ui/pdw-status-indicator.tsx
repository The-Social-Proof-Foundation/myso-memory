 ...
 *
 * Shows the current status of Memory connection.
 * V2 uses server-side Ed25519 key — no wallet connection needed.
 *
 * States:
 * - Checking: Checking Memory server health
 * - Connected: Memory server reachable and configured
 * - Unavailable: Memory not configured or server unreachable
 */
...
import { useMemoryStatus } from "../hook/use-pdw-client";

export function PDWStatusIndicator() {
  const { isConfigured } = useMemoryStatus();
    ...
    return (
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent>
            <p className="text-xs">Memory not configured (MEMORY_KEY not set)</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    );
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        ...
        <TooltipContent>
          <p className="text-xs">Memory connected — memories auto-saved</p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
