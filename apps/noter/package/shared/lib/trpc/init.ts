...
export type Context = {
  db: typeof db;
  userId: string | null;
  /** Per-user Memory delegate key (null if user has no stored key). */
  memoryKey: string | null;
  /** Per-user Memory account ID (null if user has no stored account). */
...
export const createContext = async (
  ...
  const noAuth: Context = { db, userId: null, memoryKey: null, memoryAccountId: null };
  ...
  if (zkSession?.userId && zkSession.expiresAt > new Date()) {
    const keys = await loadUsermemoryKey(zkSession.userId);
  ...
  if (walletSession?.userId && walletSession.expiresAt > new Date()) {
    const keys = await loadUsermemoryKey(walletSession.userId);
