 ...
 *    - users.mysoAddress: Derived from OAuth JWT + user salt via zkLogin derivation
 ...
 *    - Sessions expire based on maxEpoch from MySo network (typically 24 hours)
 *
 * 4. WALLET AUTHENTICATION:
 *    - Alternative to zkLogin for users with existing MySo wallets (Slush, MySo Wallet, etc.)
 *    - users.walletType: "slush" | "myso-wallet" (stored when authMethod = "wallet")
 ...
 *    - Session duration: 24 hours (independent of MySo epochs)
 *
 * 5. SESSION EXPIRATION PATTERNS:
 *    - zkLoginSessions.expiresAt: Based on maxEpoch boundary (MySo network constraint)
 ...
 *
 * 6. NOTES SYSTEM (Apple Notes / Notion-like with Memory):
 ...
 *      - Memory blockchain metadata: blobId (File Storage), graphObjectId (Nautilus), transactionId
...
export const users = pgTable(
  "users",
  {
    ...
    // MySo address (from zkLogin derivation OR wallet)
    mysoAddress: text().notNull().unique(),
    ...
    walletType: text(), // "slush" | "myso-wallet"
...
export const walletSessions = pgTable(
  "wallet_sessions",
  {
    ...
    // Wallet address (should match users.mysoAddress)
    walletAddress: text().notNull(),

    // Wallet type
    walletType: text().notNull(), // "slush" | "myso-wallet"
...
// ════════════════════════════════════════════════════════════════
// NOTE MEMORY HIGHLIGHTS (Memory blockchain integration)
// ════════════════════════════════════════════════════════════════
...
export const noteMemoryHighlights = pgTable(
  "note_memory_highlights",
  {
    ...
    blobId: text(),           // File Storage blob ID (after upload)
    graphObjectId: text(),    // Nautilus graph object ID (after indexing)
    transactionId: text(),    // MySo transaction digest
