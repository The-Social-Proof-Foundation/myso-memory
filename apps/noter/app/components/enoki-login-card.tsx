...
async function sponsoredSignAndExecute(
  ...
  const sponsorRes = await fetch(`${enokiConfig.memoryServerUrl}/sponsor`, {
    method: "POST",
  ...
  const runSetup = useCallback(
    async (address: string) => {
      ...
      try {
        ...
        const checkResult = await connectEnoki({ mysoAddress: address });
        ...
        // Derive MySo address for delegate key
        ...
        if (knownAccountId) {
          ...
          const result = await sponsoredSignAndExecute(tx, address, mysoClient, sign);
          ...
          const createResult = await sponsoredSignAndExecute(tx, address, mysoClient, sign);
          ...
          const addResult = await sponsoredSignAndExecute(tx2, address, mysoClient, sign);
    ...
    [mysoClient, signTransaction, connectEnoki],
  );
