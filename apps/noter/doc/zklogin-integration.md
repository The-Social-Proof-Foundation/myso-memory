# MySo zkLogin Integration

Noter uses MySo's zkLogin for privacy-preserving blockchain authentication. Users sign in with existing OAuth providers (Google, Facebook, etc.) and receive a unique MySo address derived from their credentials.
...
2. **Database Schema**
   - `users` table: Stores user profiles with MySo addresses
...
3. **External Services**
   ...
   - MySo Network: Epoch information and address derivation
...
```
User → Google OAuth → JWT → ZK Proof → MySo Address → Authenticated
```
...
1. **Initiate Login**
   ...
   - Fetches current epoch from MySo network
...
2. **OAuth Callback**
   ...
   - Derives MySo address from JWT + salt

3. **User Creation/Update**
   - Looks up user by MySo address
...
4. **Session Persistence**
   ...
   - Expires after ~23 hours (MySo epoch boundary)
...
function MyComponent() {
  ...
  return (
    <div>
      ...
      <p>MySo Address: {user.mysoAddress}</p>
...
# MySo zkLogin
NEXT_PUBLIC_MYSO_NETWORK=testnet
...
- Sessions expire at MySo epoch boundary (~24h)
...
The ZK proof can be used to sign MySo transactions:
...
// Submit to MySo network
await mysoClient.executeTransactionBlock({ signature, ... });
```
...
- [MySo zkLogin Documentation](https://docs.mysosocial.network/guides/developer/cryptography/zklogin-integration)
- [Mysten Labs zkLogin GitHub](https://github.com/the-social-proof-foundation/myso/tree/main/sdk/zklogin)
