...
export async function POST(request: Request) {
  ...
  try {
    const { id, message, messages, selectedChatModel, selectedVisibilityType, useMemory, memoryKey, memoryAccountId } =
      requestBody;
    ...
    const stream = createUIMessageStream({
      ...
      execute: async ({ writer: dataStream }) => {
        const result = streamText({
          model: useMemory !== false ? getMemoryModel(selectedChatModel, memoryKey, memoryAccountId) : getLanguageModel(selectedChatModel),
